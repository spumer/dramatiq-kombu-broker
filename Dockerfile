...
ENV VIRTUAL_ENV /opt/dramatiq-kombu-broker-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/dramatiq-kombu-broker/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/dramatiq-kombu-broker/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/dramatiq_kombu_broker/ && touch src/dramatiq_kombu_broker/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/dramatiq-kombu-broker/
